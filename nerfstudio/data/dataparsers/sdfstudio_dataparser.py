...
class SDFStudioDataParserConfig(DataParserConfig):
    ...
    include_depth_images:bool =True
    """whether or not to load depth images"""
    ...
    def _generate_dataparser_outputs(self, split="train"):  # pylint: disable=unused-argument,too-many-statements
        ...
        for i, frame in enumerate(meta["frames"]):
            ...
            if self.config.include_depth_images:
                depth_images_ds.append(self.config.data / frame["mono_depth_path"])
        ...
        #Automatically centralize make the original point be set to the center of scene
        # camera_to_worlds[:,0,3] = camera_to_worlds[:,0,3]- torch.mean(camera_to_worlds[:,0,3],dim = 2)
        # camera_to_worlds[:,1,3] = camera_to_worlds[:,1,3]- torch.mean(camera_to_worlds[:,1,3],dim = 2)
        # camera_to_worlds[:,2,3] = camera_to_worlds[:,2,3]- torch.mean(camera_to_worlds[:,2,3],dim = 2)
        camera_to_worlds[:,2,3] += 4
        #Automatically scale the poses to fit in +/- 1 bounding box.自动压缩场景
        scale_factor = 1.0
        if self.config.auto_scale_poses:
            scale_factor /= 1.3*(torch.max(torch.abs(camera_to_worlds[:, :3, 3]))) #添加一个系数 使场景边缘一样能被包裹

        camera_to_worlds[:, :3, 3] *= scale_factor * self.config.scale_factor
        CONSOLE.print(f"[bold yellow]T_vector_scale_factor:{scale_factor * self.config.scale_factor}")
        ...
        dataparser_outputs = DataparserOutputs(
            ...
            metadata={
                "depth_filenames": depth_images_ds if len(depth_images_ds) > 0 else None,
