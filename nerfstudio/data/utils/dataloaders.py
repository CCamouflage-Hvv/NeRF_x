...
class CacheDataloader(DataLoader):
    ...
    def _get_batch_list(self):
        ...
        # this list of batches, I guess, is a list of indices.

        indices = random.sample(range(len(self.dataset)), k=self.num_images_to_sample_from)#随机抽取新batch中的图片
        batch_list = []
        results = []

        num_threads = int(self.num_workers) * 4
        num_threads = min(num_threads, multiprocessing.cpu_count() - 1)
        num_threads = max(num_threads, 1)#让线程数大于1 小于cpu线程数

        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            for idx in indices:
                #多线程读入图像文件，每个线程有空的就读一张索引为idx的文件
                res = executor.submit(self.dataset.__getitem__, idx)#这里是要把图像文件读进去为torch.Tensor了，之前存的都还是图像文件的索引
                results.append(res)
                #这个executor.submit的返回值是多线程中的任务，这个语法的作用是把任务提交到线程池里面。
            ...
            for res in track(
                ...
                #这里的res.result是之前提交到线程池的任务的结果，调用result才开始执行任务得到结果，之前只是提交上去
                '''这里的res.result()的返回值，结构是这样的：
                res.result() = {
                    "image_idx": int 该张相片的index,
                    "image":torch.Tensor 相片文件,
                    "mask":该张相片对应的mask图片文件
                }
                一个res.result() 只包含一张相片。其实他就是Base_dataset中get_data(self, image_idx: int) -> Dict这个函数的返回值。
                '''
    ...
    def __iter__(self):
        while True:
            if self.cache_all_images:
                ...
                # trigger a reset
                self.num_repeated = 0
                collated_batch = self._get_collated_batch()#重新选取
