...
def get_surface_sliding(
    ...
    for i in range(N):
        for j in range(N):
            for k in range(N):
                ...
                points = points.reshape(cropN, cropN, cropN, 3).permute(3, 0, 1, 2) #points.shape = ([512, 512, 512, 3]) now
                if coarse_mask is not None:
                    #breakpoint()
                ...
                for _ in range(3):
                    points = avg_pool_3d(points[None])[0] #三层平均池化
                ...
                if not (np.min(z) > level or np.max(z) < level):
                    ...
                    # print(verts.min(), verts.max())
                    print(f"verts type:{type(verts)} , verts length:{len(verts)}")
                    print(f"faces type:{type(faces)} , verts length:{len(faces)}")
                    print(f"normals type:{type(normals)} , normals length:{len(verts)}")
                    meshcrop = trimesh.Trimesh(verts, faces, normals)
                    # meshcrop.export(f"{i}_{j}_{k}.ply")
                    meshes.append(meshcrop)

    combined = trimesh.util.concatenate(meshes)
    print(f"trimesh.Trimesh type: {type(trimesh.Trimesh)}")
    print(f"trimesh.base.Trimesh type: {type(trimesh.base.Trimesh)}")
    print(f"combined type:{type(combined)}")
    print(f"meshes length:{len(meshes)}")
    ...
    if return_mesh:
        ...
        if(isinstance(combined,list)):
            print("extracted mesh may be empty, marching cube failed")
